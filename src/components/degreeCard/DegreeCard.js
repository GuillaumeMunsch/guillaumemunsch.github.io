...
function DegreeCard(props) {
  ...
  const renderImg = () => (
    <Flip left={imgLeft} right={!imgLeft} duration={2000}>
      <div
        {...style_img}
        {...style({
          marginRight: imgLeft ? "50px" : "0px",
          marginLeft: !imgLeft ? "50px" : "0px",
        })}
      >
        <img
          style={{
  ...
  const renderBox = () => (
    <Fade right={imgLeft} left={!imgLeft} duration={2000} distance="40px">
      <div {...card_body}>
        <div
  ...
  return (
    <div className="degree-card" style={{ marginBottom: "30px" }}>
      {imgLeft && renderImg()}
      {renderBox()}
      {!imgLeft && renderImg()}
    </div>
